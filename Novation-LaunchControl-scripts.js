...
// todo:
// refactor LC.ledColor so that global brightness can be overridden on a per message basis

var LC = new Controller();
LC.byteArray = [];

// ---- User Settings: -------------------
LC.b = 0; // global brightness - 0 is dim, 1 is medium, 2 is bright - pick the one you want
...
// define an object with color settings for the Launch Control - see programmer's guide for explanation
// incorporates global brightness setting
LC.ledColor = {
	off: 0x0C,
	red: 0x0D+LC.b,
	amber: 0x1D+LC.b+LC.b*16,
	green: 0x1C+LC.b*16
};
...
LC.led = { // this is the customizable part
	...
	on: function (n, color, brightness, temp){
		// set some defaults if no arguments are provided
		n = n || 0;
		color = color || LC.myColor;
		brightness = brightness || LC.b;
		temp = temp || LC.t.u;

		// set up the data
		this.num = n;
		this.color = LC.ledColor[color];
		this.template = temp;
...
// LED feedback for effects
// using engine.makeConnection to bind to Mixxx controls
...
var arrow1 = engine.makeConnection('[EffectRack1_EffectUnit1]','group_[Headphone]_enable',function (v){v ? LC.led.on(10) : LC.led.off(10)});
var arrow2 = engine.makeConnection('[EffectRack1_EffectUnit2]','group_[Headphone]_enable',function (v){v ? LC.led.on(8) : LC.led.off(8)});
var arrow3 = engine.makeConnection('[EffectRack1_EffectUnit1]','group_[Channel1]_enable',function (v){v ? LC.led.on(11) : LC.led.off(11)});
var arrow4 = engine.makeConnection('[EffectRack1_EffectUnit2]','group_[Channel2]_enable',function (v){v ? LC.led.on(9) : LC.led.off(9)});;
